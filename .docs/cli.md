...
`--reset-from-example` заменяет рабочие `.env`, `databases.yaml` и `celery_workers.yaml` соответствующими `*.example`. Уже заданные ключи, пароли и другие секреты в `.env` (и модульных `.env`) не затираются: шаблон задаёт структуру и обычные настройки, затем прежние непустые секреты возвращаются на место. Команда спрашивает подтверждение; `--yes` пропускает вопрос (для скриптов). После записи пустые криптоключи текущих режимов (`API_SECRET_KEY` и остальные) генерируются заново. Учётные данные portable Postgres и Redis в `databases.yaml` сохраняются, если кластер или `redis.conf` уже есть. `databases.yaml` собирается как при `setup-full`: секция `default` и `redis` при `ERGO_BROKER=redis`; опциональные `celery` / `analytics` из example не копируются.
