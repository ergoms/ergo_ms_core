...
│   ├── user_guides/            # шпаргалки для корпуса справки (необязательно)
...
| Корпус справки платформы | `api/user_guides/*.md` и `user_description` в каталоге прав; процесс пишет пакет в `knowledge/<имя>/` |
