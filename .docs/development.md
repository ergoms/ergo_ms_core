...
**API** (Django) по умолчанию слушает порт **8000**. Запуск: `ergoms dev`. Сервер работает через **ASGI** (Django Channels) — это нужно для WebSocket (уведомления, presence, мессенджер). Если в `.env` задан `REALTIME_TRANSPORT=http_polling`, часть функций обновляется через REST без постоянного WebSocket-соединения.
