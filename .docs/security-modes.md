...
Во-первых, **человек, разворачивающий систему, не знает, какие значения безопасны**. Шаблон `.env.example` рассчитан на удобство первого запуска, поэтому в нём открыта регистрация, включены документация API и просмотр эндпоинтов в браузере, а лимит запросов для гостей — сто в минуту.
...
| `hardened` | 2 | Эксплуатация с чувствительными данными. Добавляет ротацию токенов обновления, короткие сроки жизни, обязательное шифрование канала, строгую политику содержимого страниц, пароль у Redis, обязательный журнал аудита с ненулевым сроком хранения, регистрацию только по приглашению. |
...
| `token.rotate_refresh` | нет | нет | да | да |
...
| 3 | Контроли уровней `hardened` и `maximum`: ротация токенов, второй фактор, строгая политика содержимого, проверка содержимого файлов, аудит операций чтения. | Доступны все четыре уровня. |
