...
function pip {
    ...
    Write-Host "Используйте: ergoms python-install или ergoms poetry add <пакет>" -ForegroundColor Yellow
...
function poetry {
    ...
    Write-Host "Используйте: ergoms poetry <args>, например ergoms poetry install, ergoms python-install, ergoms python-update" -ForegroundColor Yellow
...
function npm {
    ...
    Write-Host "Используйте: ergoms npm <args>, ergoms start-client, ergoms client-build, ergoms install-deps" -ForegroundColor Yellow
...
function api {
    ...
    Write-Host "Используйте: ergoms api <args> или ergoms dev, ergoms db-migrate, ergoms migrate-all, ergoms collectstatic" -ForegroundColor Yellow
...
function media_api {
    ...
    Write-Host "Используйте: ergoms media_api <args> или ergoms start-media" -ForegroundColor Yellow
...
function python {
    ...
    if ($firstArg -and ($firstArg -match 'manage\.py$' -or $firstArg -replace '\\', '/' -match '.*/manage\.py$')) {
        ...
        Write-Host "Используйте: ergoms api <команда>, например ergoms dev, ergoms db-migrate, ergoms migrate-all, ergoms collectstatic" -ForegroundColor Yellow
