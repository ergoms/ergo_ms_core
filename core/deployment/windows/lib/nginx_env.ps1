...
function Write-NginxSkipClientMessage {
    ...
    Write-ColorOutput '[OK] ergo-client-dev skipped (NGINX_ENABLED=true, client is served via nginx)' Green
    Write-ColorOutput "  Open: http://$publicHost" Cyan
    Write-ColorOutput '  After UI changes: ergoms client-build && ergoms reload-nginx' Gray
}
