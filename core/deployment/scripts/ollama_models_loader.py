...
def _parse_models_file(
    ...
    *,
    ...
    if not isinstance(raw_models, list):
        _warn(t('ollama_models_section_must_be_list', path=path))
        return []
    ...
    for index, item in enumerate(raw_models):
        if not isinstance(item, dict):
            _warn(t('ollama_models_entry_must_be_object', path=path, index=index))
            continue

        name = _resolve_model_name(item, env_values=env_values)
        if not name:
            _warn(t('ollama_models_entry_empty_name', path=path, index=index))
            continue
...
def load_resolved_models(project_root: str) -> tuple[ResolvedOllamaModel, ...]:
    ...
    for module_dir in catalog.iter_module_dirs():
        ...
        if not _module_may_declare_models(module_dir):
            _warn(
                t(
                    'ollama_models_module_not_linked',
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description=t('ollama_models_description'))
    ...
    if not agg.models:
        print(format_console('info', t('ollama_models_none_found')))
        return 0

    print(format_console('info', t('ollama_models_found', count=len(agg.models))))
